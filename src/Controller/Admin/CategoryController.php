...
use Module\Guide\Form\CategoryForm;
use Module\Guide\Form\CategoryFilter;
...
class CategoryController extends ActionController
{
	...
	public function indexAction()
    {
        ...
        foreach ($rowset as $row) {
            ...
            $list[$row->id]['url'] = $this->url('guide', array(
                'module'        => $module,
                'controller'    => 'category',
        ...
        if ($this->request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                if (Pi::service('module')->isActive('sitemap')) {
                    ...
                    if (empty($values['id'])) {
                        Pi::api('sitemap', 'sitemap')->add('guide', 'category', $row->id, $loc);
                    } else {
                        Pi::api('sitemap', 'sitemap')->update('guide', 'category', $row->id, $loc);
                    }              
                }
                // Add log
                $operation = (empty($values['id'])) ? 'add' : 'edit';
                Pi::api('log', 'guide')->addLog('category', $row->id, $operation);
