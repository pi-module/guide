...
use Module\Guide\Form\AttachForm;
use Module\Guide\Form\AttachFilter;
use Zend\Json\Json;
...
class AttachController extends ActionController
{
    ...
    protected $attachColumns = array(
        'id', 'title', 'file', 'path', 'item', 'time_create', 'size', 'type', 'status', 'hits'
    );
    ...
    public function indexAction()
    {
        ...
        foreach ($rowset as $row) {
            ...
            $item = $this->getModel('item')->find($file[$row->id]['item'])->toArray();
    ...
    public function addAction()
    {
        ...
        $id = $this->params('id');
        if (empty($id)) {
            $this->jump(array('controller' => 'item', 'action' => 'index'), __('You must select item'));
        }
        // Get item
        $item = $this->getModel('item')->find($id)->toArray();
        if (empty($item)) {
            $this->jump(array('controller' => 'item', 'action' => 'index'), __('Your selected item not exist'));
        }
        // Get all attach files
        $select = $this->getModel('attach')->select()->where(array('item' => $item['id']));
        $rowset = $this->getModel('attach')->selectWith($select);
        // Make list
        $contents = array();
        foreach ($rowset as $row) {
        ...
        $this->view()->assign('title', sprintf(__('Attach files to %s'), $item['title']));
    }
    ...
    public function editAction()
    {
        ...
        $item = $this->getModel('item')->find($file['item'])->toArray();
        // Set form
        $form = new AttachForm('attach', $item['id']);
    ...
    public function uploadAction()
    {
        ...
        if (empty($id)) {
            $return = array(
                'status' => 0,
                'message' => __('You must select item'),
            );
        } else {
            // Get item
            $item = $this->getModel('item')->find($id)->toArray();
            if (empty($item)) {
                $return = array(
                    'status' => 0,
                    'message' => __('Your selected item not exist'),
                );
            } else {
                // start upload
                ...
                if ($uploader->isValid()) {
                    ...
                    $title = $this->fileTitle($item['title'], $file);
    ...
    public function deleteAction()
    {
        ...
        if ($id) {
            ...
            $id = $row->id;
            Pi::api('item', 'guide')->attachCount($row->item);
        } else {
            $ajaxstatus = 0;
    ...
    protected function filePath($type, $path, $file)
    {
        ...
        switch ($type) {
            ...
            case 'image':
                ...
                // process image
                Pi::api('image', 'guide')->process($file, $path);
                break;
